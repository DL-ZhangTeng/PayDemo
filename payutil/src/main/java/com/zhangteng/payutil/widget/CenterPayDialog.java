...
/**
 * 支付对话框（中心弹出）
 * 已实现支付业务，生成订单id后对CenterPayDialog设置orderId即可走支付流程，设置回调OnPayResultListener获取支付结果（生成订单id与取消订单的业务未实现）
 * CenterPayDialog zhifuDialog = new CenterPayDialog(this);
 ...
 * @date 2019-06-20
 */
public class CenterPayDialog extends BaseDialog implements PayView {
    ...
    public CenterPayDialog(@NonNull Activity context) {
        super(context, R.style.SelfDialog);
        this.activity = new WeakReference<>(context);
    }

    public CenterPayDialog(@NonNull Activity context, BigDecimal balance, float amount) {
        super(context, R.style.SelfDialog);
        this.activity = new WeakReference<>(context);
        setBalance(balance);
...
//        payPresenter.getBalance();
     ...
     * 订单id
     */
    public CenterPayDialog setOrderId(String orderId) {
     ...
     * 各个模块名
     */
    public CenterPayDialog setTypeName(int typeName) {
     ...
     * @param amount 支付金额
     */
    public CenterPayDialog setPaymentAmount(float amount) {
        this.amount = amount;
        initTishi();
     ...
     * @param balance 余额
     */
    public CenterPayDialog setBalance(BigDecimal balance) {
    ...
    public CenterPayDialog setOnItemOnClickListener(OnItemOnClickListener onItemOnClickListener) {
        this.onItemOnClickListener = onItemOnClickListener;
        return this;
    }

    public CenterPayDialog setOnPayResultListener(OnPayResultListener onPayResultListener) {
        this.onPayResultListener = onPayResultListener;
        return this;
    }

    public CenterPayDialog setOnConfirmPayListener(OnConfirmPayListener onConfirmPayListener) {
        this.onConfirmPayListener = onConfirmPayListener;
        return this;
    }

    public CenterPayDialog setOnCancelPayListener(OnCancelPayListener onCancelPayListener) {
